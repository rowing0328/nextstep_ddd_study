...
class OrderTableServiceTest {
    ...
    class 주문_테이블_생성_경우 {
        ...
        void 유효한_이름을_입력하면_주문_테이블이_정상적으로_생성된다() {
            ...
            // when
            OrderTable expected = orderTableService.create(request);

            // then
            assertThat(expected.getName()).isEqualTo(DEFAULT_ORDER_TABLE_NAME);
            assertThat(expected.getNumberOfGuests()).isEqualTo(DEFAULT_NUMBER_OF_GUESTS);
    ...
    class 주문_테이블_착석_경우 {
        ...
        void 존재하는_테이블에_착석하면_테이블_상태가_사용중으로_변경된다() {
            ...
            // when
            OrderTable excepted = orderTableService.sit(created.getId());

            // then
            assertThat(excepted.isOccupied()).isTrue();
        }
    ...
    class 주문_테이블_청소_경우 {
        ...
        void 완료된_주문만_있는_테이블을_청소하면_손님수_0으로_초기화되고_상태가_사용안함으로_변경된다() {
            ...
            // when
            OrderTable excepted = orderTableService.clear(created.getId());

            // then
            assertThat(created.getNumberOfGuests()).isEqualTo(DEFAULT_NUMBER_OF_GUESTS);
            assertThat(excepted.isOccupied()).isFalse();
        }
